...
order by avg_cantidad_vendida desc
limit 1;
...
with ingresos_por_fecha as
	...
	limit 5) -- muestra los 5 dias con mas ingresos por ventas y la cantidad
...
select transactions.company.company_name,
count(transactions.transaction.id) as cantidad_transacciones,
	case when count(transactions.transaction.id) < 4 then "Sí"
	else "No"
...
group by transactions.company.id;
