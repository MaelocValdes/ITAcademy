...
use business_db;
select users.id, -- en caso de usuarios con el mismo nombre y apellido
users.name,
users.surname,
count(transactions.id) as transacciones_validas_realizadas
from users
join transactions
on users.id = transactions.user_id
where transactions.declined = 0 -- Eliminamos transacciones rechazadas
group by users.id
having transacciones_validas_realizadas > 30;
